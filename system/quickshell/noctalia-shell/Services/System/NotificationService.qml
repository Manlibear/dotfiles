...
Singleton {
  ...
  // Closes the underlying dbus notification after the toast's display
  // duration elapses, so senders (Discord etc.) see it as dismissed even
  // though it's never added to popupModel.
  Component {
    id: notificationCloserComponent
    Timer {
      property var targetNotification
      repeat: false
      onTriggered: {
        try {
          targetNotification?.dismiss();
        } catch (e) {}
        destroy();
      }
    }
  ...
  // Route real desktop notifications through the same toast UI used for
  // internal events (volume, DND, plugins) instead of the notification
  // card, for a consistent look and a reliable animation-driven dismiss.
  function showToast(notification, data) {
    notification.tracked = true;

    const duration = calculateDuration(data);
    const displayDuration = duration > 0 ? duration : 8000;
    const type = data.urgency === 2 ? "error" : "notice";
    ...
    if (actions.length > 0) {
      ...
      actionLabel = (action.text || "").trim() || "Open";
      const identifier = action.identifier;
      actionCallback = () => invokeActionAndSuppressClose(data.id, identifier);
    }

    ToastService.notify(data.appName + ": " + data.summary, data.body, "", type, displayDuration, actionLabel, actionCallback, data.originalImage || "");

    const closer = notificationCloserComponent.createObject(root, {
