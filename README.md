...
stow/
  ...
  noctalia/.config/noctalia/…  # settings.json, plugins.json, plugins/,
                             #   user-templates.toml
  kitty/.config/kitty/kitty.conf  # opacity/blur/padding + noctalia theme include
...
fish noctalia scripts hypr niri kde kitty` symlinks its contents into `$HOME`, mirroring
...
- `~/.config/kitty/current-theme.conf` and `~/.config/kitty/themes/noctalia.conf`
  — same story, kitty's own theme-kitten mechanism (the `BEGIN_KITTY_THEME`
  block in `kitty.conf`) regenerates these live from the active theme.
  `~/.config/kitty/kitty.conf.bak` is kitty's own auto-backup, also not config.
...
- The visage IR-camera udev rule (`/etc/udev/rules.d/99-visage-ir.rules`) is
  generated by `install.sh` at install time, not tracked in the repo — the
  IR camera's USB `ID_PATH` differs per machine, so a static file only ever
  worked on one laptop (learned this going from the Flow X16 to the Zenbook
  S16). Detected by picking the second `video4linux` node that reports
  capture capability (RGB webcam is always the first) — a heuristic, not a
  hardware guarantee. If detection picks the wrong device, `visaged` stays
  up but fails to authenticate (PAM falls through to your password, nothing
  breaks) — check `udevadm info -q property --name=/dev/videoN` per node and
  fix the detection loop in `install.sh` if a third machine doesn't fit the
  pattern.
